...
const hAssert = require('../../test/utils/hassert');
const { getHtmlRPCAddress, getDefaultHtmlAddress, getWalletPassphrase, isWalletEncrypted } = require('../../test/utils');

const { assert } = chai;

console.log(`Your HTML RPC address is ${getHtmlRPCAddress()}`);
console.log(`Your Default HTML address is ${getDefaultHtmlAddress()}`);

describe('Hweb3', () => {
  const HTML_ADDRESS = getDefaultHtmlAddress();
  let hweb3;

  beforeEach(() => {
    hweb3 = new Hweb3(getHtmlRPCAddress());
  });
  ...
  describe('getBlockchainInfo()', () => {
    it('returns the block info', async () => {
      const res = await hweb3.getBlockchainInfo();
      assert.isDefined(res);
      assert.isObject(res);
  ...
  describe('getBlockCount()', () => {
    it('returns the blockcount', async () => {
      const res = await hweb3.getBlockCount();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('getBlockHash()', () => {
    it('returns the block hash', async () => {
      const res = await hweb3.getBlockHash(0);
      assert.isDefined(res);
      assert.isString(res);
    });
  });
  ...
  describe('getHexAddress()', () => {
    it('returns the hex address', async () => {
      const hexDecodedAddress = bs58.decode(HTML_ADDRESS).toString('hex');
      const hexadecimalAddress = await hweb3.getHexAddress(HTML_ADDRESS);
  ...
  describe('fromHexAddress()', () => {
    it('returns the html address', async () => {
      const htmlAddress = await hweb3.fromHexAddress('17e7888aa7412a735f336d2f6d784caefabb6fa3');
      assert.isString(htmlAddress);
      assert.lengthOf(htmlAddress, 34);
    });
  });
  ...
  describe('validateAddress()', () => {
    it('returns an object validating the address', async () => {
      let res = await hweb3.validateAddress(HTML_ADDRESS);
      assert.isDefined(res);
  ...
  !_.includes(process.argv, '--cleanenv') ? describe.skip : describe('cleanEnv tests', () => {
    describe('getNewAddress()', () => {
      it('returns a new html address', async () => {
