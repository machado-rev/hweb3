...
class Decoder {
  static toHtmlAddress(hexAddress, isMainnet = false) {
    ...
    let hAddress = hexAddress;
    // Add network byte
    if (isMainnet) {
      hAddress = MainnetNetworkByte + hAddress;
    } else {
      hAddress = TestnetNetworkByte + hAddress;
    }

    const hAddressBuffer = Buffer.from(hAddress, 'hex');
    // Double SHA256 hash
    const hash1 = crypto.createHash('sha256').update(hAddressBuffer).digest('Hex');
    const hash1Buffer = Buffer.from(hash1, 'hex');
    const hash2 = crypto.createHash('sha256').update(hash1Buffer).digest('Hex');

    // get first 4 bytes
    hAddress += hash2.slice(0, 8);

    // base58 encode
    const address = bs58.encode(Buffer.from(hAddress, 'hex'));
    return address;
  }
